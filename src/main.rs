...
impl AppState {
    fn new(schema: Schema<Query, Mutation, Subscription>) -> Self {
        Self { schema }
    }
}
...
impl AxumAppState for AppState {
    fn routes(&self) -> Router {
        let preflight_middleware =
            ResponseHttpHeaderMutatorLayer::new(|_req_headers, res_headers| {
                res_headers.insert(
        ...
        Router::new()
            ...
            .route(
                "/graphql",
                post_service(GraphQL::new(self.schema.clone()))
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::Serve(params) => {
            ...
            let mut app = AxumApp::new(AppState::new(schema));
